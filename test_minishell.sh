...
echo "[INFO] heredoc tests are manual-only in this script (heredoc reads from /dev/tty)"

# --------------------------------------------------
# 4.1 MULTIPLE REDIRECTIONS
# --------------------------------------------------
echo ""
echo "--- 4.1 Multiple Redirections ---"

run_test "multiple output redirects: last wins" "echo first > /tmp/ms_mr_a.txt > /tmp/ms_mr_b.txt
cat /tmp/ms_mr_b.txt" "first"

run_test "multiple input redirects: last wins" "echo one > /tmp/ms_mr_in1.txt
echo two > /tmp/ms_mr_in2.txt
cat < /tmp/ms_mr_in1.txt < /tmp/ms_mr_in2.txt" "two"

run_test "input and output in same command" "echo hello > /tmp/ms_mr_src.txt
cat < /tmp/ms_mr_src.txt > /tmp/ms_mr_dst.txt
cat /tmp/ms_mr_dst.txt" "hello"

run_test "append chain keeps latest line" "echo a > /tmp/ms_mr_append.txt
echo b >> /tmp/ms_mr_append.txt
echo c >> /tmp/ms_mr_append.txt
cat /tmp/ms_mr_append.txt" "c"

run_test "append after input redirect" "echo start > /tmp/ms_mr_hd.txt
echo extra > /tmp/ms_mr_src2.txt
cat < /tmp/ms_mr_src2.txt >> /tmp/ms_mr_hd.txt
cat /tmp/ms_mr_hd.txt" "extra"
...
# --------------------------------------------------
# 7. PIPE ENV INHERITANCE
# --------------------------------------------------
echo ""
echo "--- 7. Pipe Env Inheritance ---"
...
run_test "extract_redirect_filename: simple filename" "echo content > /tmp/ms_simple.txt
cat /tmp/ms_simple.txt" "content"
...
run_test "append mode basic" "echo line1 > /tmp/ms_append.txt
echo line2 >> /tmp/ms_append.txt
cat /tmp/ms_append.txt" "line2"

run_test "input redirect reads file" "echo content > /tmp/ms_input_test.txt
cat < /tmp/ms_input_test.txt" "content"

run_test "command with redirect and args" "echo first second > /tmp/ms_args.txt
cat /tmp/ms_args.txt" "first second"
